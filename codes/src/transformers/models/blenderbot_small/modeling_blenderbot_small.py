...
class BlenderbotSmallModel(BlenderbotSmallPreTrainedModel):
    def __init__(self, config: BlenderbotSmallConfig):
        ...
        # self.graph = GraphNetwork(2*config.D, config.classes, config.relations, config.max_seq_len, config.graph_hidden_size)
    ...
    def forward(
        ...
        if comet_ids is not None:
            comet_outputs = self.encoder(
                input_ids=comet_ids,
                attention_mask=comet_attention_mask,
                return_dict=return_dict,
            )
            len = encoder_outputs[0].size()[1]
            comet_outputs = comet_outputs[0][:,-1,:].unsqueeze(1).repeat(1, len, 1)
            encoder_outputs[0] = encoder_outputs[0] + comet_outputs
            # features, edge_index, edge_norm, edge_type, edge_index_lengths = self.batch_graphify(encoder_outputs[0], mask, seq_length)
            # graph_out = self.graph_net(features, edge_index, edge_norm, edge_type, seq_lengths, umask)
        # decoder outputs consists of (dec_features, past_key_value, dec_hidden, dec_attn)
        decoder_outputs = self.decoder(
            input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask,
            encoder_hidden_states=graph_out,
