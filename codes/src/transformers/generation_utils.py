...
class GenerationMixin:
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        comet_outputs = encoder(
            input_ids=encoder_kwargs["comet_ids"],
            attention_mask=encoder_kwargs["comet_attention_mask"],
        )
        len = input_ids.size()[1]
        comet_outputs = comet_outputs[0][:, -1, :].unsqueeze(1).repeat(1, len, 1)
        # model_kwargs["encoder_outputs"]: ModelOutput = encoder(input_ids, return_dict=True, **encoder_kwargs)
        combine_encoder = encoder(input_ids, attention_mask=encoder_kwargs["attention_mask"], return_dict=True)
        combine_encoder.last_hidden_state += comet_outputs
        model_kwargs["encoder_outputs"]: ModelOutput = combine_encoder
        return model_kwargs
